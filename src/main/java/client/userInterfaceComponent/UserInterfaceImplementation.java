...
public class UserInterfaceImplementation implements UserInterface {
    ...
    public void verlasseRaum(String roomName) throws RoomNotFoundException, GivenObjectNotValidException, NameNotValidException, InterruptedException {
        ...
        String url = GlobalConstantsAndValidation.BASE_URL + GlobalConstantsAndValidation.SERVER_IP_ADRESS + ":" + GlobalConstantsAndValidation.SERVER_HTTP_PORT + GlobalConstantsAndValidation.SERVER_ROOM_RESOURCES + "/" + roomName + GlobalConstantsAndValidation.SERVER_ROOM_USER + "/" + GlobalConstantsAndValidation.USER.getUserName();
        try {
            rt.delete(url);
    ...
    public void loggeEin(String userName, int port, String ipAdress) throws GivenObjectNotValidException, IOException {
        ...
        try {
            System.out.println("Versuche mit dem Server zu verbinden.");
            rt.postForObject(url, request2, String.class);
            System.out.println("Sie wurden mit dem User\n" + GlobalConstantsAndValidation.USER + "\nTCPPort=" + environment.getProperty("local.server.port") + " eingeloggt.");
        } catch (HttpClientErrorException e) {
            System.out.print("Der Server konnte nicht erreicht werden. " + e.getStatusCode());
            throw new GivenObjectNotValidException(e.getStatusCode() + ": " + e.getResponseBodyAsString());
        }
